...
function _roDatos(){
  ...
  (DB.edpProveedores||[]).forEach(r=>{
    ...
    const a=fila(r.eqId);
    a.prov+=+r.subtotal||0;a.desc+=+r.montoDesc||0;a.nEdp++;
...
function rResultadoOperativo(mantenerFoco){
  ...
  const filas=D.filas.map(a=>{
    ...
    return`<tr style="border-bottom:1px solid var(--border)">
      ...
      <td style="${TD};text-align:right;font-family:monospace;color:${a.prov>0?_RO_C_PROV:'var(--muted)'};font-weight:${a.prov>0?'700':'400'}">${a.prov>0?_roN(a.prov):'—'}</td>
...
function _roExcel(){
  ...
  const HDR=['Código','Equipo','Tipo','Proveedor','N° EDP','Proveedor S/','Galones','Combustible S/','Costo Total S/','% comb.'];
  const aoa=[
  ...
  D.filas.forEach(a=>{
    ...
    aoa.push([S(a.codigo,{b:1}),S(a.nombre),S(a.tipo),S(a.nEdp?a.proveedor:'Propio / sin EDP'),
      S(a.nEdp,{al:'center'}),S(a.prov,{al:'right',numFmt:'#,##0.00',col:'C2410C'}),
  ...
  ws['!cols']=[{wch:14},{wch:28},{wch:16},{wch:26},{wch:8},{wch:14},{wch:11},{wch:15},{wch:15},{wch:9}];
...
function _roPrint(){
  ...
  const filas=D.filas.map(a=>`<tr>
    ...
    <td style="${TD};text-align:right;color:#C2410C;font-weight:700">${a.prov>0?_roN(a.prov):'—'}</td>
